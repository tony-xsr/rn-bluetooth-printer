...
public class RNBluetoothEscposPrinterModule extends ReactContextBaseJavaModule
    ...
    public void printText(String text, @Nullable  ReadableMap options, final Promise promise) {
        try {
            ...
            int fonttype=0;
            if(options!=null) {
                encoding = options.hasKey("encoding") ? options.getString("encoding") : "iso-8859-1";
            ...
            String toPrint = text;
            Log.e("printText",toPrint);

           /** if ("UTF-8".equalsIgnoreCase(encoding)) {
            byte[] b = text.getBytes("UTF-8");
                 toPrint = new String(b, Charset.forName(encoding));
             }**/
           // byte[] bytes = PrinterCommand.POS_Print_Text(toPrint, encoding, 0, widthTimes, heigthTimes, fonttype);
            byte[] bytes = toPrint.getBytes("iso-8859-1");
